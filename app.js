...
if (!loaded.ok) { $('#storageError').hidden = false; $('#storageError').textContent = 'Silo không đọc được dữ liệu đang lưu và đã dừng ghi để tránh ghi đè.'; } else {
  ...
  document.addEventListener('focusout', event => { if (!['expenseAmount', 'incomeAmount'].includes(event.target.id)) return; const dialog = event.target.closest('dialog'); if (event.relatedTarget instanceof Element && event.relatedTarget.closest('.money-accessory')) { updateKeyboardInset(); return; } window.setTimeout(() => { const active = document.activeElement; if (!dialog?.contains(active) || (!['expenseAmount', 'incomeAmount'].includes(active.id) && !active.closest('.money-accessory'))) dialog?.querySelector('.money-accessory')?.setAttribute('hidden', ''); updateKeyboardInset(); }, 150); });
  document.addEventListener('click', event => { const button = event.target.closest('[data-suggested-amount]'); if (!button) return; const accessory = button.closest('.money-accessory'); const input = document.getElementById(accessory.dataset.input); input.value = formatMoneyInput(button.dataset.suggestedAmount); input.setSelectionRange(input.value.length, input.value.length); dirtyDialogId = input.closest('dialog')?.id ?? dirtyDialogId; suppressSuggestionsOnFocus = input; input.focus({ preventScroll: true }); updateAmountSuggestions(input); window.setTimeout(() => { if (suppressSuggestionsOnFocus === input) suppressSuggestionsOnFocus = null; }, 0); });
  document.addEventListener('input', event => { if (!['expenseAmount', 'incomeAmount'].includes(event.target.id)) return; const value = formatMoneyInput(event.target.value); event.target.value = value; event.target.setSelectionRange(value.length, value.length); dirtyDialogId = event.target.closest('dialog')?.id ?? dirtyDialogId; updateAmountSuggestions(event.target); }); document.querySelectorAll('[data-close]').forEach(button => button.addEventListener('click', () => { const dialog = document.getElementById(button.dataset.close); if (dirtyDialogId === dialog.id && !confirm('Bỏ các thay đổi chưa lưu?')) return; dialog.close(); })); document.querySelectorAll('dialog').forEach(dialog => dialog.addEventListener('close', () => { dirtyDialogId = null; dialog.querySelector('.money-accessory')?.setAttribute('hidden', ''); }));
  let swipeEndedAt = 0;
  const endSwipe = () => {
