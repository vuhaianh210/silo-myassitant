...
function selectedBounds() { return periodBounds(state.selectedPeriodKey, state.cycleStartDay); }
...
function saveCycleStartDay(event) { event.preventDefault(); const day = Number($('#cycleStartDay').value); if (!Number.isInteger(day) || day < 1 || day > 31) { $('#cycleStartDayError').textContent = 'Ngày bắt đầu từ 1 đến 31.'; return; } const finish = () => { try { repository.saveCycleStartDay(day); state.cycleStartDay = day; state.selectedPeriodKey = periodKeyForDate(todayKey(), day); state.selectedCategoryId = 'all'; $('#periodSettingsSheet').close(); render(); announce('Đã đổi ngày bắt đầu kỳ'); } catch { reportStorageError(); } }; if (day === state.cycleStartDay) { finish(); } else confirmAction(`Đổi ngày bắt đầu sang ngày ${day}? Các khoản chi cũ sẽ được nhóm lại theo kỳ mới nhưng không bị xóa.`, 'Đổi ngày').then(ok => { if (ok) finish(); }); }
